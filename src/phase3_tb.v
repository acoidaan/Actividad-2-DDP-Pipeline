// ============================================================
// Testbench Fase 3 multiciclo
// Reutiliza el knapsack_full_tb pero con paths locales (override
// de los parametros MEM_FILE/INIT_FILE de memprog y regfile).
//
// Verifica LOAD/STORE/CALL/RET integrados con todo el resto.
// ============================================================
...
task pulsar_y_esperar;
    ...
    begin
        ...
        // 1. Esperar a que la CPU este lista (LEDG = 0xAA)
        i = 0;
        ...
        if (i >= 20000) begin
            ...
            // 3. Esperar a que detecte la pulsacion (LEDG=0x01)
            i = 0;
            ...
            // 5. Esperar a terminar (LEDG=0x80)
            i = 0;
            while (LEDG !== 8'h80 && i < 30000) begin @(posedge clk); i = i + 1; end
            if (i >= 30000) begin
...
initial begin
    ...
    if (!fail) $display("=== FASE 3: PASS ===");
    else       $display("=== FASE 3: FAIL ===");
    $finish;
end
